...
This project (`expedition-api-service`) serves as the starting skeleton.
...
1.  **Clone the Repository:**
    ...
    cd expedition-api-service
    ```

2.  **Check out the Lab Branch:**
    *The `lab1-start` branch contains the project skeleton, ready to go.*
    ```
    git checkout lab1-start
    ```

3.  **Launch External Services (Database):**
    *We are launching the database (`training-database`) and the mock API (`external-legacy-api`) in the background.*
    *Note: We use the modern V2 command (with a space).*
    ```
...
4.  **Open the Project in IntelliJ:**
    Open the `expedition-api-service` folder in IntelliJ IDEA.

5.  **Run the Application (Locally):**
    * Find the `ExpeditionApiServiceApplication.java` file.
...
**A:** The database container isn't running. You missed step 3. Run `docker-compose up -d training-database` *before* running the application from IntelliJ.